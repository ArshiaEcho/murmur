...
const ModelCard: React.FC<ModelCardProps> = ({
  ...
  const baseClasses =
    "flex flex-col rounded-2xl px-4 py-3.5 gap-2 text-left border border-line bg-card transition-[border-color,background-color,transform,box-shadow] duration-200 outline-none focus-visible:ring-2 focus-visible:ring-signal focus-visible:ring-offset-2 focus-visible:ring-offset-bg";

  const getVariantClasses = () => {
    if (status === "active") {
      return "border-signal bg-signal-soft";
    }
    if (isFeatured) {
  ...
  const getInteractiveClasses = () => {
    ...
    return "cursor-pointer hover:border-signal hover:bg-card-2 hover:shadow-[var(--shadow)] hover:scale-[1.01] active:scale-[0.99] group";
  };
  ...
  return (
    ...
    >
          ...
          <div className="hidden sm:flex items-center ms-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <p className="text-xs text-text-3 w-24 text-end">
                  {t("onboarding.modelCard.accuracy")}
                </p>
                <div className="w-16 h-1.5 bg-line-2 rounded-full overflow-hidden">
                  <div
      ...
      <div className="flex items-center gap-3 w-full -mb-0.5 mt-0.5 h-5">
        ...
        {showModelSize && (
          <span className="flex items-center gap-1.5 ms-auto text-xs text-text-3">
            {status === "downloadable" ? (
