...
fn run_event_tap(
    ...
    //
    // KEYBOARD ONLY (Strat fork). We deliberately exclude all mouse events. This
    // tap runs in Default (active) mode so it can swallow the bare `fn` hotkey;
    // an active tap delivers every masked event to this callback INLINE on the OS
    // input path, so any mouse event left in the mask can stall or drop clicks
    // under CPU load. The mouse arms of `event_tap_callback` only observe (they
    // never block), so removing these types costs zero functionality while
    // keeping the pointer responsive. `fn` is a modifier => arrives via
    // FlagsChanged, which is retained, so hold-to-talk and double-tap-to-lock are
    // unaffected.
